...
const OneRoom=({selectRoom, showReservation})=>{
  ...
  const handleDates=(newcheckInDate, newcheckOutDate)=>{
    setCheckInDate(newcheckInDate)
    setCheckOutDate(newcheckOutDate)
  }

  const addReservation =()=>{
   axios.post("http://localhost:3100/api/hotel/addReservation", values).then((res)=>{
     showReservation()
   })
  ...
  return(
   <div className='oneRoom'>
    ...
    <div className='calendar'>
      ...
      onCheckInOutChange={()=>handleDates}
    />
    </div>
    <div className='total'>
      <h4>Total Price:{selectRoom.price}</h4>
      </div>
    <button className='BookRoom' onClick={()=>{addReservation()}} >Book Now</button>
